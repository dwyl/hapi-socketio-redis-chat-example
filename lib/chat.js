(function() {
	...
	var redisClient = redis.createClient(rc.port, rc.host, {no_ready_check: true}); // create client
	redisClient.auth(rc.auth)
	var pub         = redis.createClient(rc.port, rc.host, {no_ready_check: true}); // create client
	pub.auth(rc.auth)
	var sub         = redis.createClient(rc.port, rc.host, {no_ready_check: true}); // create client
	sub.auth(rc.auth)
	...
	function chatHandler (socket) {
		...
		socket.on('io:message', function (msg) {
			redisClient.HGET("people", socket.client.conn.id, function (err, name) {
				...
				redisClient.RPUSH("chat:messages", str);   // chat history
