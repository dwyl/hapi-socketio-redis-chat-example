...
function chatHandler (socket) {
  ...
  socket.on('io:name', function (name) {
    ...
    pub.HSET("currentUsers", socket.client.conn.id,name);
    // console.log(socket.client.conn.id + " > " + name + ' joined chat!');
    pub.publish("chat:people:new", name);
    // socket.emit('currentUsers', function(users){
    //   pub.HG
    // })
    //publishes that message so it's available to everyone subscribing to redis. good for horizontal scaling
  });
  ...
  socket.on('disconnect', function(name){
    ...
    pub.HDEL("currentUsers", socket.client.conn.id, function(){
